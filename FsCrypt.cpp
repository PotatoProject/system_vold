...
bool is_wrapped_key_supported_external(int flags) {
    if (is_ice_supported_external(flags))
        return GetEntryForMountPoint(&fstab_default, DATA_MNT_POINT)->fs_mgr_flags.wrappedkey;
    return false;
}

bool is_metadata_wrapped_key_supported() {
    return GetEntryForMountPoint(&fstab_default, METADATA_MNT_POINT)->fs_mgr_flags.wrappedkey;
}
